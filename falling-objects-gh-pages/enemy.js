...
//var player =  new component(30, 30, "smiley.gif", 10, 120, "image");
...
function drawPlayer() {
	contxt.beginPath();
	//contxt.clearRect(0, 0, c.width, c.height); 
  
	contxt.rect(player.x, player.y, player.size, player.size);
	//contxt.drawImage(img, 50, 100);
	//contxt.fillStyle = player.color;
	contxt.fillStyle = pat;
...
function draw(){
	...
	if(!gameOver){
		...
		document.getElementById("canvas").style.backgroundImage = "gameover.jpg";
